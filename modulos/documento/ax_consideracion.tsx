...
const formReducer = (state: ConsideracionModel, event: any): ConsideracionModel => {
    if (event.FORM_DATA) {
        return { ...event.FORM_DATA }
    }
    if (event.FORM_ADD) {
        return new ConsideracionModel()
    }
    return { ...state, [event.name]: event.value }
}

export default function AxConsideracion({ ID, setID, setEstadoEdicion }: TypeFormularioProps) {
    const [formData, setFormData] = useReducer(formReducer, new ConsideracionModel());
    ...
    useEffect(() => {
        ...
        if (ID == 0) {
            ...
            const fetchData = async () => {
                const response = await fetch(`/api/documento/consideracion/${ID}`);
                const data: ConsideracionModel = await response.json();
        ...
        <>
            <nav className="flex items-start pb-1 sm:hidden" aria-label="Breadcrumb">
                <button
                    ...
                    <span>Lista de Consideraciones</span>
                </button>
            </nav>
