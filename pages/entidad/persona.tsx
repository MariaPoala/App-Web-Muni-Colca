...
export default function AxPagePersona() {
    const [ID, setID] = useState(-1)
    const [lista, setLista] = useState<PersonaModel[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const response = await fetch(`/api/entidad/persona`, { method: "GET" })
                ...
                <div className="min-h-0 flex-1 flex overflow-hidden ">
                    <main className="min-w-0 flex-1 border-t border-gray-200 xl:flex">
                        ...
                        <div className={((estadoEdicion == EnumEstadoEdicion.SELECCIONADO || estadoEdicion == EnumEstadoEdicion.EDITANDO) ? "block" : "hidden sm:block") + " flex-1 inset-y-0 pl-0 m-1 sm:pl-72 md:pl-80 lg:pl-80 bg-white"}>
                            {ID == -1
                                ? <AxInicio nombre="Persona" />
                                : <AxPersona ID={ID} setID={setID} setEstadoEdicion={setEstadoEdicion} />
                            }
                        </div>
                        {/*LISTA*/}
                            ...
                            <div className="h-full relative flex flex-col border-r border-gray-200 bg-gray-100">
                                ...
                                <div className="flex-shrink-0">
                                    <div className="px-6 pt-2 pb-2 ">
                                        <h2 className="text-lg font-medium text-gray-900">Lista de Personas</h2>
                                    ...
                                    <div className="bg-white shadow overflow-hidden sm:rounded-md">
                                        ...
                                        {textoFiltro !== '' && listaFiltro.length === 0 && (
                                            <div className="py-14 px-4 text-center sm:px-14">
                                                ...
                                                <p className="mt-4 text-sm text-gray-900">No se encontraron personas usando ese término de búsqueda.</p>
                                            </div>
                                        )}
                                    </div>
                                </nav>
